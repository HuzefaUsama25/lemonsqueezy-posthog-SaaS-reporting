	...
	return (
		<div className="p-4 md:p-8 space-y-8">
				...
				<div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
					...
					<div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
						<div className="flex items-center space-x-2">
							<Switch id="view-mode" checked={viewMode === 'advanced'} onCheckedChange={handleViewModeChange} />
				...
				<div className="flex flex-col items-center justify-center space-y-4 mb-8">
					<div className="text-center">
						...
						<p className="text-sm text-muted-foreground mt-1">
							{latestData
								? new Date(latestData.date).toLocaleDateString(undefined, {
										weekday: 'long',
										year: 'numeric',
										month: 'long',
										day: 'numeric',
					...
					<div className="flex items-center space-x-2">
						<Switch id="view-mode-simple" checked={false} onCheckedChange={() => handleViewModeChange(true)} />
						<Label htmlFor="view-mode-simple">Advanced</Label>
					</div>
				</div>
			)}
									...
									<div className="flex items-center gap-2 mt-1 text-[9px] text-muted-foreground">
										<span className="whitespace-nowrap">
											...
											<span className="font-semibold text-primary/80">
												${(latestData.revenue - latestData.renewalRevenue).toLocaleString()}
											</span>
										</span>
										<span className="text-border">|</span>
										<span className="whitespace-nowrap">
											Ren: <span className="font-semibold text-primary/80">${latestData.renewalRevenue.toLocaleString()}</span>
										</span>
									</div>
								)}
							</CardContent>
						...
						<div className="w-[55%] relative group">
							<div className="h-16 flex items-center justify-between px-4 border-2 border-green-500/20 rounded-xl shadow-sm bg-green-50/50 dark:bg-green-900/10 relative z-10">
								...
								<span className="text-xl font-bold text-green-700 dark:text-green-400">
									{latestData?.purchases.toLocaleString() || '0'}
								</span>
							</div>
						</div>
					</div>
				...
				<>
					<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold">${data.reduce((acc, curr) => acc + curr.revenue, 0).toLocaleString()}</div>
						...
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold">${data.length > 0 ? data[data.length - 1].mrr.toFixed(2) : '0.00'}</div>
						...
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold">{data.length > 0 ? data[data.length - 1].activeCustomers : 0}</div>
						...
						<Card>
							...
							<CardContent>
								<div className="text-2xl font-bold">{data.reduce((acc, curr) => acc + curr.visitors, 0).toLocaleString()}</div>
								<p className="text-xs text-muted-foreground">In selected period</p>
							</CardContent>
						</Card>
					</div>
					...
					<Card className="col-span-4">
						<CardHeader>
							...
							<CardDescription>Compare visitors, interactions, and revenue over time.</CardDescription>
						</CardHeader>
			...
			<div className="fixed bottom-4 left-4 right-4 md:hidden z-50">
				<Button className="w-full shadow-lg" size="lg" onClick={handleRefresh} disabled={loading}>
